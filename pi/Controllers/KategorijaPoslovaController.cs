...
namespace FirmaMvc.Controllers
{
    public class KategorijaPoslovaController : Controller
    {
        ...
        public IActionResult Index(string filter, int page = 1, int sort = 1, bool ascending = true)
        {
            ...
            KategorijaPoslovaFilter kpf = new KategorijaPoslovaFilter();
        if (!string.IsNullOrWhiteSpace(filter)){
        kpf = KategorijaPoslovaFilter.FromString(filter);
        if (!kpf.IsEmpty())
        {
          if (kpf.Id.HasValue)
          {
            kpf.Naziv = ctx.KategorijaPoslova
                                .Where(kpf => kpf.Id == kpf.Id)
                                .Select(kpf => kpf.Naziv)
                                .FirstOrDefault();
          }
          query = kpf.Apply(query);
        }
      }
